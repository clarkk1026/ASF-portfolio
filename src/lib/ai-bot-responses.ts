...
const isTeamMemberQuestion = (q: string, tokens: string[]) => {
	if (matches(q, [/yuki|mory|sue|ioanis|le wei|alex dean|alex/])) return true;
	if (hasWord(tokens, ['yuki', 'mory', 'sue', 'ioanis', 'wei', 'alex', 'dean'])) {
		return true;
	}
	if (
		matches(q, [
			/who (is|are) (yuki|sue|le wei|alex)/,
			/tell me about (yuki|sue|le wei|alex|the team)/,
			/about (yuki|sue|le wei|alex)/,
		])
	) {
		return true;
	}
